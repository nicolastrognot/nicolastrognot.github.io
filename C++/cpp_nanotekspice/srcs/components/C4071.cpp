...
#include "../../inc/components/C4071.hpp"
...
nts::Tristate nts::C4071::Compute(size_t pin_num_this)
{
    if (this->output_pins.find(pin_num_this) == this->output_pins.end())
        throw (PinError(std::string("Pin n°").append(std::to_string(pin_num_this)).append(" (Chipset n°4071 : ").append(this->name).append(") is not computable.")));
    if (this->pins.find(this->output_pins[pin_num_this][0]) == this->pins.end() ||
        ...
        throw (PinError(std::string("Pin n°").append(std::to_string(pin_num_this)).append(" (Chipset n°4071 : ").append(this->name).append(") is not correctly wired.")));
    ...
    this->tmp[pin_num_this] = nts::do_or(a, b);
    return (this->tmp[pin_num_this]);
}
...
{
    std::cout << this->name << " (4071)" << std::endl;
}

nts::C4071::C4071(std::string const &name) : name(name)
{
