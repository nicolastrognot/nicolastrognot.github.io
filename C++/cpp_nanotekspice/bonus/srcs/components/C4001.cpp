...
#include "../../inc/components/C4001.hpp"
...
nts::Tristate nts::C4001::Compute(size_t pin_num_this)
{
    if (this->output_pins.find(pin_num_this) == this->output_pins.end())
        throw (PinError(std::string("Pin n°").append(std::to_string(pin_num_this)).append(" (Chipset n°4001 : ").append(this->name).append(") is not computable.")));
    if (this->pins.find(this->output_pins[pin_num_this][0]) == this->pins.end() ||
        ...
        throw (PinError(std::string("Pin n°").append(std::to_string(pin_num_this)).append(" (Chipset n°4001 : ").append(this->name).append(") is not correctly wired.")));
    ...
    this->tmp[pin_num_this] = nts::do_nor(a, b);
    return (this->tmp[pin_num_this]);
}
...
nts::C4001::C4001(std::string const &name) : name(name)
{
